...
#callout("Teacher planning check", [
  One target. One clean model. Several guided responses. Independent work only after the class data justify it.
])
...
#three-col(
  ...
  small-box("Current-unit retrieval"),
)
...
Prerequisite atoms I need today:

#write-box(height: 0.5in)

= I Do: Clean Worked Example

Problem:

#write-box(height: 0.45in)

Teacher model and decision points:

#write-box(height: 1.35in)
...
#two-col(
  [
    #small-box("Example A: same structure", height: 1.05in)
    #small-box("What should we check?", height: 0.45in)
  ],
  [
    #small-box("Example B: one feature changed", height: 1.05in)
    #small-box("Likely error", height: 0.45in)
  ],
)

= You Do: Short Independent Practice

Complete these when released by the teacher.

#write-box(height: 1.0in)

= Exit Ticket

#write-box(height: 0.65in)
